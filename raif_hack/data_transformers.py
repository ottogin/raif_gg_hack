    ...
    def fit(self, X: pd.DataFrame, y: typing.Union[np.array, pd.Series] = None):
        ...
        for col in self.categorical_features:
            self.mean_price_by_cat[col] = (
                X.groupby(col)[self.target]
                .apply(lambda x: self.smoothed_target_encoding(x))
                .fillna(self.mean_price)
            )
    ...
    def transform(self, X: pd.DataFrame, y: typing.Union[np.array, pd.Series] = None):
        ...
        if self.__is_fitted:
            ...
            for col in self.categorical_features:
                ...
                X_cp[new_col] = (
                    X_cp[col].map(self.mean_price_by_cat[col]).fillna(self.mean_price)
                )
